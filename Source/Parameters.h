...
namespace Parameters
{
	...
	static AudioProcessorValueTreeState::ParameterLayout createParameterLayout()
	{
        ...
        //parameters.push_back(std::make_unique<AudioParameterFloat>(ParameterID(nameDownSample, 1), "Sample Rate", NormalisableRange<float>(400.0f, 44100.0f, 1.0f, 0.4f), defaultSampleRate));
        parameters.push_back(std::make_unique<AudioParameterFloat>(ParameterID(nameBitCrush, 1), "Bits", NormalisableRange<float>(1.0f, 20.0f, 0.001f, 0.4f), defaultBitDepth));
        //parameters.push_back(std::make_unique<AudioParameterFloat>(ParameterID(nameFreqDS, 1), "LFO Frequency DownSampling (Hz)", NormalisableRange<float>(0.01f, 60.0f, 0.01f, 0.2f), defaultFreq));
        parameters.push_back(std::make_unique<AudioParameterFloat>(ParameterID(nameFreqBC, 1), "LFO Frequency BitCrush (Hz)", NormalisableRange<float>(0.01f, 60.0f, 0.1f, 0.2f), defaultFreq));
        //parameters.push_back(std::make_unique<AudioParameterFloat>(ParameterID(nameAmountDS, 1), "LFO Amount DownSampling (%)", NormalisableRange<float>(0.0f, 100.0f, 0.1f, 1.0f), defaultAmount));
        parameters.push_back(std::make_unique<AudioParameterFloat>(ParameterID(nameAmountBC, 1), "LFO Amount BitCrush (bits)", NormalisableRange<float>(0.0f, 4.0f, 0.01f, 1.0f), defaultAmount));
